...
export function InvoicesTab({ onNavigate }: { onNavigate?: (tab: string) => void } = {}) {
  const { invoices, activeTenant, customers, cancelInvoice, transmitInvoice, currentUser, bulkTransmitInvoices, updateInvoice, deleteInvoice, refreshAll, retryInvoice, retryBulkInvoices, isBgRefreshing } = useHub() as any;
            ...
            <div className="overflow-y-auto p-6 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={()=>{ setEditError(null); const newLine={ itemCode:`SKU-${Date.now().toString().slice(-4)}`, description:'New line', quantity:1, unitPrice:1000, hsOrServiceCode:'SRV-7212.10', vatRate:7.5 }; setEditingInvoice({...editingInvoice, lineItems:[...(editingInvoice.lineItems||[]), newLine]}); }} className="px-2 py-1 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-[11px] font-semibold flex items-center gap-1"><Plus className="w-3 h-3" /> Add line</button>
                </div>
                  ...
                  {(editingInvoice.lineItems || []).map((li:any, idx:number)=>(
                    <div key={idx} className="grid grid-cols-12 gap-2 bg-white p-2 rounded-lg border border-slate-200">
                      ...
                      <div className="col-span-1 flex items-center justify-center">
                        <button onClick={()=>{ const n=editingInvoice.lineItems.filter((_:any,i:number)=>i!==idx); if(n.length===0) { setEditError('At least one line required'); return; } setEditingInvoice({...editingInvoice, lineItems:n}); }} className="p-1 bg-white hover:bg-rose-50 text-slate-400 hover:text-rose-600 border border-slate-200 rounded-lg"><Trash2 className="w-3 h-3" /></button>
                      </div>
                    </div>
                  ))}
                </div>
