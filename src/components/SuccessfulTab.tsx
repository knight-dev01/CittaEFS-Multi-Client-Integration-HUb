...
export function SuccessfulTab({ onNavigate }: { onNavigate?: (tab: string) => void } = {}) {
  const { invoices, activeTenant, currentUser } = useHub() as any;
  ...
  const successfulInvoices = tenantInvoices.filter(inv => ['APPROVED', 'SIGNED'].includes(inv.status));

  const filteredInvoices = successfulInvoices.filter(inv => {
    const matchesSearch =
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl p-5 border border-slate-200/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 font-sans">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-xs font-medium border border-slate-200 rounded-lg bg-slate-50/50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all text-slate-900"
          />
        </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 overflow-hidden shadow-sm font-sans">
        <div className="px-5 py-4 bg-emerald-50 text-emerald-900 border-b border-emerald-100 flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            <CheckCircle2 className="w-4 h-4 text-emerald-600" />
            <h3 className="text-sm font-bold tracking-tight">
              {activeTenant.name} Successful Invoices ({filteredInvoices.length})
            </h3>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-emerald-700 hidden sm:inline">
              Format: <strong className="text-emerald-900">{activeTenant.platformType}</strong>
            </span>
          </div>
        </div>
        ...
        {filteredInvoices.length === 0 ? (
          <div className="p-12 text-center text-slate-500 font-sans">
            ...
            <p className="text-sm font-semibold text-slate-800">No successful invoices match your filter.</p>
            <p className="text-xs text-slate-500 mt-1">Transmit invoices to see them appear here once APPROVED/SIGNED.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-emerald-50 text-emerald-700 font-semibold text-[11px] uppercase tracking-wider border-b border-emerald-100">
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {filteredInvoices.map((inv) => {
                  ...
                  return (
                    <tr key={inv.id} className={`hover:bg-emerald-50/30 transition-colors ${isExpanded ? 'bg-emerald-50/50' : ''}`}>
                      ...
                      <td className="py-3 px-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 text-[10px] font-semibold rounded-full ${inv.status === 'SIGNED' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' :
                            'bg-emerald-50 text-emerald-700 border border-emerald-200'
                          }`}>
                          {inv.status === 'SIGNED' && <CheckCircle2 className="w-3 h-3 mr-1 text-emerald-600" />}
                          {inv.status}
                        </span>
                      </td>
        ...
        {/* Per-row overlay for line items */}
        {expandedInvoiceId && (() => {
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setExpandedInvoiceId(null)}>
              <div className="bg-white w-full max-w-3xl rounded-2xl border border-slate-200 shadow-2xl overflow-hidden max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-emerald-50">
                  ...
                  <button onClick={() => setExpandedInvoiceId(null)} className="p-1.5 hover:bg-white rounded-lg text-slate-500 hover:text-slate-700 border border-transparent hover:border-slate-200 cursor-pointer"><ChevronRight className="w-4 h-4 rotate-90" /></button>
                </div>
                <div className="overflow-y-auto p-4 space-y-3">
                    ...
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100 text-slate-700">
                        {inv.lineItems.map((li) => (
                          <tr key={li.id} className="hover:bg-emerald-50/30">
                            <td className="py-2 px-3 font-mono font-semibold text-emerald-700">{li.itemCode}</td>
                            ...
                            <td className="py-2 px-3 font-mono text-[11px]"><span className={`px-2 py-0.5 rounded-full border text-[10px] font-semibold ${li.hsOrServiceCode==='UNMAPPED' ? 'bg-rose-50 text-rose-700 border-rose-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>{li.hsOrServiceCode}</span></td>
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    {inv.irn && <button onClick={() => setQrModalInvoice(inv)} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-semibold cursor-pointer inline-flex items-center gap-1.5"><QrCode className="w-3.5 h-3.5" /> View QR</button>}
                  </div>
                </div>
              </div>
            </div>
        ...
        {qrModalInvoice && (
          <div className="fixed inset-0 bg-slate-900/80 z-50 flex items-center justify-center p-4">
            <div className="bg-white border-4 border-slate-900 max-w-md w-full p-6 text-slate-900 text-center space-y-4 font-mono">
              <div className="w-12 h-12 bg-emerald-400 border-2 border-slate-900 text-slate-950 flex items-center justify-center mx-auto">
              ...
              <div className="flex justify-center space-x-3 pt-2">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center space-x-1.5 px-4 py-2 bg-slate-900 hover:bg-slate-800 text-emerald-400 text-xs font-black uppercase border-2 border-slate-900"
                >
