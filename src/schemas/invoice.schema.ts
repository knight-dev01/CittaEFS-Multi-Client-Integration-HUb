...
export const invoiceLineItemSchema = z.object({
  ...
  codeType: z.enum(['HS_CODE', 'SERVICE_CODE', 'UNMAPPED']).optional().default('SERVICE_CODE'),
  vatRate: z.number().min(0).max(100).default(16)
});

// Full Invoice Validation Schema
export const invoiceIngestionSchema = z.object({
  tenantId: z.string().min(1, 'Tenant ID is required'),
  clientInvoiceNumber: z.string().min(1, 'Client Invoice Number is required'),
  invoiceType: z.enum(['STANDARD', 'CREDIT_NOTE', 'DEBIT_NOTE', 'CANCELLATION']).default('STANDARD'),
  ...
  if (effectiveKind === 'B2B' && (!data.customerTin || data.customerTin.trim().length === 0)) {
    effectiveKind = 'B2C';
  }
  ...
  const subtotal = Number(transformedLineItems.reduce((acc, item) => acc + item.taxableAmount, 0).toFixed(2));
  const totalVat = Number(transformedLineItems.reduce((acc, item) => acc + item.vatAmount, 0).toFixed(2));
  ...
  const rawPayloadHash = crypto.createHash('sha256').update(payloadJson).digest('hex');
