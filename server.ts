...
async function startServer() {
  ...
  // Registration endpoint - only works if ENABLE_REGISTRATION is set or no users exist
  app.post('/api/auth/register', async (req, res) => {
    try {
      ...
      if (userCount > 0 && !enableRegistration) {
        return res.status(403).json({ success: false, error: 'Registration is disabled. Please contact an administrator.' });
      }
